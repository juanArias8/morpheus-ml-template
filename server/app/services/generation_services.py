...
class GenerationServices:
    ...
    def generate_text2img_images(
        self, db: Session, request: ImageGenerationRequest, email: str
    ) -> str:
        backend_request = self._build_ray_image_request(
            db=db, request=request, email=email
        )
        return self.generator.generate_text2img_images(request=backend_request)

    def generate_img2img_images(
        self, db: Session, request: ImageGenerationRequest, image: bytes, email: str
    ) -> str:
        backend_request = self._build_ray_image_request(
            db=db, request=request, email=email
        )
        return self.generator.generate_img2img_images(
    ...
    def generate_inpainting_images(
        ...
        return self.generator.generate_inpainting_images(
            request=backend_request, image=image, mask=mask
        )

    def generate_text_with_chatbot(
        self, db: Session, request: TextGenerationRequest, email: str
    ) -> str:
        backend_request = self._build_ray_text_request(
            db=db, request=request, email=email
        )
        return self.generator.generate_text_with_chatbot(request=backend_request)

    def _get_user_and_model(self, db: Session, model_handler: str, email: str):
        ...
        db_model = self.model_repository.get_model_by_handler(
            db=db, handler=model_handler
        )
        if not db_model:
            raise ModelNotFoundError(f"model {model_handler} does not exist in db")
        return db_user, db_model

    def _build_ray_image_request(
        self, db: Session, request: Union[ImageGenerationRequest], email: str
    ) -> ImageGenerationRequest:
        db_user, db_model = self._get_user_and_model(
            db=db, model_handler=request.handler, email=email
        )
        request_dict = {
            **request.dict(),
            "user_id": db_user.id,
